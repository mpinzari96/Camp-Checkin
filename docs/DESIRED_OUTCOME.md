...
A fast, reliable, mobile-first PWA that lets multiple volunteers check campers in
simultaneously with no conflicts, no page refreshes, and minimal taps — polished
...
1. **No duplicate check-ins.** Check-in is a two-state model: a camper is either
   **Checked in** or **Not checked in** (there is no checked-out state). State changes
   go ONLY through the atomic Postgres RPCs (`check_in`, `undo_check_in`). Undo exists
   to reverse a mistaken check-in. Never write `checked_in_at` directly from the client.
...
4. **Security holds.** RLS stays on. Volunteers can check in (with undo), add walk-ins,
   and toggle a registrant's liability status; only admins edit full profiles, delete,
   export, or view the audit log. The service-role key never ships to the browser
   (never in a `NEXT_PUBLIC_` var).
5. **Every critical action is audited** (check-in, undo check-in, liability toggle,
   edits, deletes, walk-ins) with who + when. Audit writes are enforced through the
   atomic RPCs so a code path can't silently skip them.
...
- **Schema convention:** `supabase/schema.sql` is kept consolidated — it always reflects
  the *current* desired state (no obsolete columns/RPCs left in it). Numbered migrations
  (`supabase/0NN_*.sql`) exist to move *already-deployed* databases forward. A fresh
  install runs `schema.sql` and arrives at current state directly, so schema and code
  never diverge even if migrations aren't replayed.
...
- Liability: set manually in-app only, via a profile toggle through the audited
  `set_liability` RPC — no automatic import.
...
- No re-introduction of a checked-out state or an automatic liability-import path without
  a deliberate decision (both were intentionally removed).
